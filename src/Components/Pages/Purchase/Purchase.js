...
const Purchase = () => {
  ...
  useEffect(() => {
    fetch("https://airplane-server-side.onrender.com/products")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handlePurchaseSubmit = (e) => {
    ...
    // send to the server
    fetch("https://airplane-server-side.onrender.com/orders", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="container">
      <h1 className="text-primary">
        Please, give your right info for purchase product.
      </h1>
      <h1>{product?.name}</h1>
